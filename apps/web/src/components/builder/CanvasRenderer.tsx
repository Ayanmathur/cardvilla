...
import { CanvasJson, CanvasElement } from './types';
import styles from './CanvasRenderer.module.css';

interface CanvasRendererProps {
  canvasJson: CanvasJson;
  data?: Record<string, any>; // Live card values key-value pairs
  interactive?: boolean; // If true, clickable links work
  scale?: number;
  className?: string;
...
export const CanvasRenderer: React.FC<CanvasRendererProps> = ({
  ...
  const renderMotifAnimation = (element: CanvasElement) => {
    ...
    if (animationType === 'shine_sweep') {
      return <div className={styles.shineSweep} />;
    }
  ...
  const renderElementContent = (element: CanvasElement) => {
    // If element is bound to a FieldSchema, check if live `data` overrides it
    const fieldKey = element.fieldBinding?.fieldKey;
    const liveValue = fieldKey && data[fieldKey] !== undefined ? data[fieldKey] : null;

    switch (element.type) {
      case 'text': {
        const textVal = liveValue !== null ? String(liveValue) : element.content || 'Sample Text';
      ...
      case 'logo': {
        ...
        return (
          <img
            src={imgSrc}
            alt={element.fieldBinding?.label || 'Card Element'}
            style={{
              width: '100%',
              height: '100%',
              objectFit: 'contain',
      ...
      case 'button': {
        ...
        return <div style={buttonStyle}>{btnText}</div>;
      }

      case 'motif': {
        return renderMotifAnimation(element);
      }
  ...
  return (
    <div
      className={`${styles.canvasOuter} ${className}`}
      style={{
    ...
    >
      ...
      >
        {/* Render shine sweep if background is gold/silver foil */}
        ...
        {elements.map((el) => {
          ...
          return (
            ...
            >
              ...
              {/* Field Schema Badge in Builder view */}
              {el.fieldBinding && !interactive && (
                <div className={styles.fieldBadge} title={`Bound to: ${el.fieldBinding.fieldKey}`}>
                  {el.fieldBinding.fieldKey} ({el.fieldBinding.fieldType})
                </div>
              )}
            </div>
          );
