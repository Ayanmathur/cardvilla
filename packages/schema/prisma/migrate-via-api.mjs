// Execute migration SQL against Supabase via their pg-meta API
const sql = `
-- CreateEnum
...
async function run() {
  ...
  const SERVICE_ROLE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFyenNhc2R5aWJoY2NnaXRrbWpxIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NTc4MTI1NSwiZXhwIjoyMTAxMzU3MjU1fQ.nHOo_vDcYcEdjb76tYWakp1mibyrrC2NpE-cGsZqPWs';

  console.log('🗄️  Executing migration SQL via Supabase API...\n');
  ...
  if (!resp.ok) {
    console.log('RPC endpoint returned:', resp.status, await resp.text());
    ...
    if (resp2.ok) {
      ...
      console.log('✅ Migration executed via pg-meta!');
      console.log(JSON.stringify(result, null, 2).substring(0, 500));
    } else {
      console.log('pg-meta returned:', resp2.status, await resp2.text());
      
      // Try the management API SQL endpoint
      console.log('\nTrying management API SQL endpoint...');
      const resp3 = await fetch(`${SUPABASE_URL}/rest/v1/rpc/exec_sql`, {
        method: 'POST',
        headers: {
          'apikey': SERVICE_ROLE_KEY,
          'Authorization': `Bearer ${SERVICE_ROLE_KEY}`,
      ...
      if (resp3.ok) {
        ...
        console.log('exec_sql returned:', resp3.status, await resp3.text());
        console.log('\n❌ API migration failed. Please run the SQL manually in the Supabase SQL Editor.');
        console.log('Go to: https://supabase.com/dashboard/project/arzsasdyibhccgitkmjq/sql/new');
      }
    }
  } else {
