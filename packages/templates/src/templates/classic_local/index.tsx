...
import { classicLocalSchema } from './schema';
import styles from './classic_local.module.css';
...
const CornerFlourish = () => (
  <svg viewBox="0 0 40 40" fill="none" stroke="currentColor" strokeWidth="1.5">
    <path d="M4 4h16C12 4 4 12 4 20V4z" fill="currentColor" fillOpacity="0.1"/>
    <path d="M4 4c8 0 16 8 16 16"/>
...
export const ClassicLocal: React.FC<TemplateProps> = ({ data }) => {
  const d = data || {};

  const fullName = d.full_name || 'Rajesh Sharma';
  const companyName = d.company_name || 'Sharma Heritage Traders';
  const title = d.title || 'Proprietor';
  const phone = d.phone || '+91 98765 43210';
  const whatsapp = d.whatsapp || '+91 98765 43210';
  const email = d.email || 'contact@sharmatraders.com';
  const address = d.address || '45 Main Market, Chandni Chowk, Delhi';
  const website = d.website || 'www.sharmatraders.com';
  const logo = d.logo;

  const ctaSaveContact = d.cta_save_contact || 'Save Contact';
  const ctaCall = d.cta_call || 'Call Us';
  const ctaDirections = d.cta_directions || 'Get Location';
  ...
  return (
    <div className={styles.container}>
      ...
      >
        <div className={styles.doubleFrame} />
        <div className={styles.doubleFrameInner} />

        <div className={styles.flourishTopLeft}><CornerFlourish /></div>
        <div className={styles.flourishTopRight}><CornerFlourish /></div>
        <div className={styles.flourishBottomLeft}><CornerFlourish /></div>
        <div className={styles.flourishBottomRight}><CornerFlourish /></div>
...
registerTemplate({
  component: ClassicLocal,
  schema: classicLocalSchema,
  meta
});

export default ClassicLocal;
