...
import { silverEleganceSchema } from './schema';
import { shineSweep } from '../../utils/animations';
import styles from './silver-elegance.module.css';
...
export const SilverElegance: React.FC<TemplateProps> = ({ data }) => {
  const d = data || {};

  const fullName = d.full_name || 'Victoria Sterling';
  const companyName = d.company_name || 'Sterling & Co. Fine Timepieces';
  const title = d.title || 'Principal Consultant';
  const tagline = d.tagline || 'Precision Engineering & Design • Est. 2010';
  const phone = d.phone || '+1 888 444 3322';
  const whatsapp = d.whatsapp || '+1 888 444 3322';
  const email = d.email || 'victoria@sterlingtime.com';
  const address = d.address || '100 Financial Center Plaza, Suite 4000, New York, NY';
  const website = d.website || 'www.sterlingtime.com';
  const logo = d.logo;

  const ctaSaveContact = d.cta_save_contact || 'Save Contact';
  const ctaCall = d.cta_call || 'Call Office';
  ...
  return (
    <div className={styles.container}>
      <div className={styles.content}>
        {/* Silver Border Card Container with Shine Sweep */}
        <div className={styles.silverBorderWrapper}>
          <motion.div
...
// Register template
registerTemplate({
  component: SilverElegance,
  schema: silverEleganceSchema,
  meta,
});

export default SilverElegance;
